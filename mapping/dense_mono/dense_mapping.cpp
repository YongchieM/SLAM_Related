...
int main(int argc, char **argv) {
    ...
    for (int index = 1; index < color_image_files.size(); index++) {
        ...
        SE3d pose_T_C_R = pose_curr_TWC.inverse() * pose_ref_TWC;   // 坐标转换>关系： T_C_W * T_W_R = T_C_R
...
bool epipolarSearch(
    ...
    if (half_length > 100) half_length = 100;   // 不希望搜索太多东西
...
double NCC(const cv::Mat& ref, const cv::Mat& curr, const Vector2d& pt_ref, const Vector2d& pt_curr)
{
    // 零均值-归一化互相关
    // 计算均值
