...
int main(int argc, char **argv){
    ...
    for(int i=0; i<nPoints; i++){
        ...
        double depth = fx*baseline/disparity; //calculate depth
        depth_ref.push_back(depth);
        pixels_ref.push_back(Eigen::Vector2d(x,y));
    }
    
    //estimate 01~05.png's pose using this information
...
void DirectPoseEstimationSingleLayer(const cv::Mat& img1, const cv::Mat& img2, const VecVector2d& px_ref, const vector<double> depth_ref, Sophus::SE3d& T21)
{
    ...
    for(int iter=0; iter<iterations; iter++){
        ...
        if(std::isnan(update[0])){
            //with a black or white patch and H is irreversible
    ...
    for(size_t i=0; i<px_ref.size(); ++i){
        ...
        if(p_cur[0]>0 &&  p_cur[1]>0){
            cv::circle(img2_show,Point2f(p_cur[0],p_cur[1]),2,Scalar(0,250,0),2);
            cv::line(img2_show,Point2f(p_ref[0],p_ref[1]),Point2f(p_cur[0],p_cur[1]),Scalar(0,250,0));
        }
    }
    imshow("current",img2_show);
    waitKey();
}
