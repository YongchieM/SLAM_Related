 ...
 * img1:the first image [in]
 * img2:the second image [in]
 * kp1:keypoints in img1  [in]
 * kp2:keypoints in img2, if empty, use initial guess in kp1 [in|out]
 * success:true if a keypoint is tracked successfully [out]
 * inverse:use inverse formulation? [in]
 **/
 ...
 * img1:the first pyramid [in]
 * img2:the second pyramid [in]
 * kp1:keypoints in img1 [in]
 * kp2:keypoints in img2 [out]
 * success:true if a keypoint is tracked successfully [out]
 * inverse:set true to enable inverse formulation [in]
 **/
...
void OpticalFlowSingleLevel(const cv::Mat& img1, const cv::Mat& img2, const vector<cv::KeyPoint>& kp1, vector<cv::KeyPoint>& kp2, vector<bool>& success, bool inverse, bool has_initial)
{
    kp2.resize(kp1.size());
...
void OpticalFlowTracker::calculateOpticalFlow(const cv::Range& range)
{
    ...
    for(size_t i=range.start; i<range.end; i++){
        ...
        bool succ = true; //indicate if this  point succeed
        ...
        for(int iter=0; iter<iterations; iter++){
            ...
            for(int x=-half_patch_size; x<half_patch_size; x++)
                for(int y=-half_patch_size; y<half_patch_size; y++){
                    ...
                    if(inverse==false){
                        ...
                        //this J does not change when dx, dy is updated, so we can store it and compute error
                        J = -1.0*Eigen::Vector2d(
            ...
            if(std::isnan(update[0])){
                //a black or white patch and H is irreversible
